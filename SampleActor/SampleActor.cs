...
namespace SampleActor
{
    ...
    internal class SampleActor : Actor, ISampleActor
    {
        ...
        public async Task<string> Reverse(string val, CancellationToken cancellationToken)
        {
            var count = await StateManager.GetStateAsync<int>("count", cancellationToken);
            count++;

            char[] charArray = val.ToCharArray();
            Array.Reverse(charArray);
            var result = string.Format("{0}: {1}", count, new string(charArray));
